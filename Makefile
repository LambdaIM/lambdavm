...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
ETHERMINT_BINARY = lambdavm
ETHERMINT_DIR = lambdavm
BUILDDIR ?= $(CURDIR)/build
SIMAPP = ./app
HTTPS_GIT := https://github.com/LambdaIM/lambdavm.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=lambda \
...
PACKAGE_NAME:=github.com/LambdaIM/lambdavm
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/LambdaIM/lambdavm/types"
	godoc -http=:6060
...
test-import:
	go test -run TestImporterTestSuite -v --vet=off github.com/LambdaIM/lambdavm/tests/importer
